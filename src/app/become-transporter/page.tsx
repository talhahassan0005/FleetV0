...
import { Truck, CheckCircle2, MapPin, Award, BarChart3, Phone, MessageCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
          ...
          <div className="text-center space-y-6">
            <FadeIn direction="up" delay={0} duration={800} distance={20} blur={true}>
              <div className="inline-block px-6 py-2.5 bg-gradient-to-r from-emerald-500/20 to-blue-500/20 border border-emerald-500/30 rounded-full backdrop-blur-sm">
                <span className="text-emerald-400 text-sm font-semibold tracking-wide">Join Our Network</span>
              </div>
            </FadeIn>
            ...
            <FadeIn direction="up" delay={300} duration={900} distance={30} blur={true}>
              <p className="text-lg md:text-xl text-blue-100 max-w-2xl mx-auto">
                Join 500+ verified transporters. Get consistent loads. Increase your revenue.
              </p>
            </FadeIn>
            ...
            <FadeIn direction="up" delay={600} duration={900} distance={40} blur={true}>
              <div className="grid grid-cols-3 gap-6 pt-12 mt-8 border-t border-blue-800/30">
                <div className="text-center">
                  ...
                  <p className="text-blue-200 text-sm mt-2">Active Transporters</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl md:text-4xl font-bold text-emerald-400">50K+</p>
                  <p className="text-blue-200 text-sm mt-2">Loads/Month</p>
                </div>
                <div className="text-center">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-emerald-100 text-sm mt-2">Revenue Growth</p>
            </div>
            <div>
              <p className="text-3xl font-bold">24/7</p>
              <p className="text-emerald-100 text-sm mt-2">Support Team</p>
            </div>
            <div>
              <p className="text-3xl font-bold">No Fees</p>
              <p className="text-emerald-100 text-sm mt-2">Commission Structure</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn direction="up" delay={0} duration={800} distance={30}>
            <h2 className="text-3xl md:text-4xl font-bold text-center text-slate-900 mb-12">
              Why Join FleetXchange?
            </h2>
          </FadeIn>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: BarChart3, title: "Consistent Load Flow", desc: "Regular freight opportunities" },
              { icon: CheckCircle2, title: "Easy Onboarding", desc: "Quick verification process" },
              { icon: Award, title: "Professional Network", desc: "Work with trusted shippers" },
              { icon: MapPin, title: "Build Your Route", desc: "Work routes that suit you" },
              { icon: Truck, title: "Real-Time Updates", desc: "Live tracking for all loads" },
              { icon: MessageCircle, title: "Dedicated Support", desc: "We're always available" }
            ].map((item, i) => (
      ...
      <div className="fixed bottom-4 right-4 flex flex-col gap-3 z-30">
        <a
          href="https://wa.me/27738281478?text=Hi%20FleetXchange%2C%20I%20want%20to%20become%20a%20transporter"
          target="_blank"
...
function ApplicationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/transporter', {
        method: 'POST',
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {success && (
        <div className="bg-emerald-100 border-2 border-emerald-400 text-emerald-800 px-4 py-4 rounded-lg font-semibold">
          ✓ Application received! We&apos;ll review it and contact you soon.
        </div>
      )}
      ...
      <input
        type="text"
        name="vehicleTypes"
        placeholder="Vehicle Types (e.g., Box trucks, Flatbeds, etc.)"
      ...
      <input
        ...
        placeholder="Operating Routes (e.g., JNB-CPT, KZN-JNB)"
