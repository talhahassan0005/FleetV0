...
export default async function AdminDashboard() {
  ...
  const [total, pending, active, delivered, recentLoads, unverified] = await Promise.all([
    prisma.load.count(),
    prisma.load.count({ where: { status: { in: ['PENDING', 'QUOTING', 'QUOTED', 'APPROVED'] } } }),
    prisma.load.count({ where: { status: 'IN_TRANSIT' } }),
  ...
  return (
    <>
      ...
      <PageLayout>
        {unverified > 0 && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 border-l-4 border-l-amber-500 rounded text-amber-800 text-sm flex items-center justify-between">
            <span>⚠ {unverified} user{unverified > 1 ? 's' : ''} awaiting verification.</span>
            <Link href="/admin/users" className="btn-navy text-[10px] px-2 py-1">Review</Link>
          </div>
        )}
